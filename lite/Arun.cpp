...
* FN:		CMLTOKENIZE
* CR:		08/18/08 AM.
* SUBJ:	Runtime variant of CML tokenize pass.
* NOTE:	Interpreted/compiled call the same optimized tokenizer.
********************************************/

bool Arun::cmltokenize(
	Parse *parse,
	int num									// Pass number.
	)
{
...
* FN:		DICTTOK
* CR:		07/29/11 AM.
* SUBJ:	Runtime variant of DICT tokenize pass.
* NOTE:	Interpreted/compiled call the same optimized tokenizer.
********************************************/
...
* FN:		DICTTOKZ
* CR:		08/16/11 AM.
* SUBJ:	Runtime variant of DICT tokenize pass (ZAP WHITESPACE).
* NOTE:	Interpreted/compiled call the same optimized tokenizer.
********************************************/
...
dicttokz.setZapwhite(true);	// SKIP WHITESPACE IN PARSE TREE.	// 08/16/11 AM.
if (!dicttokz.Execute(parse,0))
	return false;
...
* FN:		CHECKEND
* CR:		05/20/08 AM.
* SUBJ:	Runtime variant of checkEnd.
* NOTE:	Operates after successful check region.
*			If there's a label generated, put it after that.
********************************************/

bool Arun::checkend(Nlppp *nlppp)
{
return Pat::checkEnd(nlppp);
}
...
* FN:		CHECKFAIL
* CR:		09/15/08 AM.
* SUBJ:	Abnormal end to CHECK region.
* NOTE:	e = 1 means FAIL.
*			e = 2 means EXITPASS
********************************************/

bool Arun::checkfail(Nlppp *nlppp, int e)
{
Pat::checkFinal(false,nlppp);	// Cleanups for check region.
if (e == 1)
	nlppp->fail_ = true;
else if (e == 2)
	nlppp->exitpass_ = true;
return false;	// Rule failed to match.
}
...
* FN:		SUCCEED
* CR:		09/15/08 AM.
* SUBJ:	Runtime function for succeed.
* NOTE:	
********************************************/

bool Arun::succeed(Nlppp *nlppp)
{
nlppp->succeed_ = true;
throw ex_SUCCEED;
return true;
...
* FN:		FAIL
* CR:		09/15/08 AM.
* SUBJ:	Runtime function for fail.
* NOTE:	
********************************************/

bool Arun::fail(Nlppp *nlppp)
{
nlppp->fail_ = true;
throw ex_FAIL;
return true;
...
{
// If no nodes matched, rule match has failed.	// FIX.			// 05/19/08 AM.
if (!nlppp->first_)										// FIX.			// 05/19/08 AM.
	return false;											// FIX.			// 05/19/08 AM.

// Note: The compiled code is not good about which region we are in.	// 08/09/08 AM.
nlppp->region_ = REGPOST;	// Going to post region next.		// 08/09/08 AM.
...
bool Arun::match_simple_dejunk(NODE *node, const ELT *elt, Nlppp *nlppp)
{
...
bool Arun::match_singlet_dejunk(NODE *node, const ELT *elt, Nlppp *nlppp)
{
Pn *pn;
bool success = false;
long len;
...
/********************************************
* FN:		MATCH_TREE_DEJUNK
* CR:		09/09/11 AM.
* SUBJ:	Runtime match of elt.
********************************************/

bool Arun::match_tree_dejunk(NODE *node, const ELT *elt, Nlppp *nlppp)
{
if (!match_tree_dejunk(node, elt->name, true, nlppp))
	return false;
if (!elt->except.reg && !elt->except.spec)
	return true;
return !match_tree_list_dejunk(node,elt->except,true, nlppp);
	return false; 
}

// Recursive workhorse.
bool Arun::match_tree_dejunk(NODE *node, const _TCHAR *name, bool root, Nlppp *nlppp)
{
if (!node)
	return false;
...
if (match_tree_dejunk(((Node<Pn> *)node)->pDown, name, false, nlppp))	// Recurse down.
	return true;
...
if (!root && !(((Node<Pn> *)node)->pLeft))
	...
	for (node = ((Node<Pn> *)node)->Right();
		  ...
		  node = ((Node<Pn> *)node)->Right())
		{
		if (match_tree_dejunk(node,name,false,nlppp))
			return true;
		}
	}
return false;
...
// Recursive workhorse.
bool Arun::match_tree_list_dejunk(
	NODE *node,
	ELIST elist,
	bool root,
	Nlppp *nlppp
...
if (!root && !(((Node<Pn> *)node)->pLeft))
	...
	for (node = ((Node<Pn> *)node)->Right();
		  ...
		  node = ((Node<Pn> *)node)->Right())
		{
		if (match_tree_list_dejunk(node,elist,false,nlppp))
			return true;
		}
	}
return false;
...
/********************************************
* FN:		MATCH_LIST_DEJUNK
* CR:		09/09/11 AM.
* SUBJ:	Runtime match of elt.
* NOTE:	Nonwild, nonlist, nonspecial.
********************************************/

bool Arun::match_list_dejunk(NODE *node, const ELT *elt, 
	Nlppp *nlppp
	)
{
Pn *pn = &(((Node<Pn> *)node)->data);
...
/********************************************
* FN:		MATCH_FAILS_DEJUNK
* CR:		09/09/11 AM.
* SUBJ:	Runtime match of elt with FAILS list.
* NOTE:	Nonwild, nonlist, nonspecial.
********************************************/

bool Arun::match_fails_dejunk(NODE *node, const ELT *elt, 
	Nlppp *nlppp
	)
{
Pn *pn = &(((Node<Pn> *)node)->data);
...
/********************************************
* FN:		MATCH_LIST_TREE_DEJUNK
* CR:		09/09/11 AM.
* SUBJ:	Runtime match of elt.
* NOTE:	Nonwild, nonlist, nonspecial.
********************************************/

bool Arun::match_list_tree_dejunk(NODE *node, const ELT *elt, 
	Nlppp *nlppp
	)
{
if (!match_tree_list_dejunk(node,elt->match,true,nlppp))
	return false;
...
/********************************************
* FN:		MATCH_FAILS_TREE_DEJUNK
* CR:		09/09/11 AM.
* SUBJ:	Runtime match of elt with FAILS list.
* NOTE:	Nonwild, nonlist, nonspecial.
********************************************/

bool Arun::match_fails_tree_dejunk(NODE *node, const ELT *elt, 
	Nlppp *nlppp
	)
{
if (!match_tree_list_dejunk(node,elt->fail,true,nlppp))
	return true;
if (!elt->except.htab && !elt->except.reg && !elt->except.spec)
	return false;
if (match_tree_list_dejunk(node,elt->except,true,nlppp))
	return true;
return false;
}
...
/********************************************
* FN:		MATCH_LIST_SINGLET_DEJUNK
* CR:		09/09/11 AM.
* SUBJ:	Runtime match of elt.
* NOTE:	Nonwild, nonlist, nonspecial.
********************************************/

bool Arun::match_list_singlet_dejunk(NODE *node, const ELT *elt, 
	Nlppp *nlppp
	)
{
Pn *pn;
...
/********************************************
* FN:		MATCH_FAILS_SINGLET_DEJUNK
* CR:		09/09/11 AM.
* SUBJ:	Runtime match of elt with FAILS list.
* NOTE:	Nonwild, nonlist, nonspecial.
********************************************/

bool Arun::match_fails_singlet_dejunk(NODE *node, const ELT *elt, 
	Nlppp *nlppp
	)
{
Pn *pn;
	...
	switch ((enum specialType) *ptr)
		...
		case XCAP:		// _xCAP
			if (micro_cap(node, 0, nlppp))						// FIX.	// 08/09/02 AM.
				return true;											// FIX.	// 08/09/02 AM.
			break;
		case XCAPLET:		// _xCAPLET											// 07/10/09 AM.
			if (micro_caplet(node, 0, nlppp))								// 07/10/09 AM.
				return true;														// 07/10/09 AM.
			break;
		case XLET:		// _xLET													// 07/10/09 AM.
			if (micro_let(node, 0, nlppp))									// 07/10/09 AM.
				return true;														// 07/10/09 AM.
			break;
		case XEND:		// _xEND
...
bool Arun::special_simple_dejunk(
	NODE *node,
	const ELT *elt,
	MICROFN match,
	Nlppp *nlppp
...
bool Arun::special_singlet_dejunk(
	NODE *node,
	const ELT *elt,
	MICROFN match,
	Nlppp *nlppp
...
// If something matched to the left of here, we can't go there, but it's
// not the start of the current phrase.				// FIX.	// 08/10/08 AM.
if (nlppp->first_)											// FIX	// 08/10/08 AM.
	{
	if (nlppp->first_->pLeft)								// FIX.	// 08/10/08 AM.
		return false;											// FIX.	// 08/10/08 AM.
	return true;												// FIX.	// 08/10/08 AM.
	}

// If no nodes matched yet, presumably the below is ok?	// 08/10/08 AM.
...
bool Arun::micro_let(NODE *node, const ELT *elt, Nlppp *nlppp)
{
Pn *pn = &(((Node<Pn> *)node)->data);
_TCHAR *x = pn->getName();
if (!*x || *(x+1))
	return false;	//	Length 0 or greater than 1.
return (pn->getType() == PNALPHA);
}
...
bool Arun::micro_caplet(NODE *node, const ELT *elt, Nlppp *nlppp)
{
Pn *pn = &(((Node<Pn> *)node)->data);
_TCHAR *x = pn->getName();
return (*x && !*(x+1) && (pn->getType() == PNALPHA)
	&& is_upper((_TUCHAR)*x));
}
...
if (!Pat::args_range(from,to,nlppp->coll_,nlppp->len_,nlppp,
...
if (!Pat::args_range(from,to,nlppp->coll_,nlppp->len_,nlppp,/*UP*/ nfirst,nlast))
	return false;
...
// Setup restart for recursive passes.					// FIX.		// 09/11/07 AM.
tmp->restart_ = tmpf->restart_;							// FIX.		// 09/11/07 AM.
...
tmp->ruleline_ = (nlppp->sugg_ ? nlppp->sugg_->ruleline : 0);	// 09/15/08 AM.
...
if (!coll1)	// 11/20/09 AM.
	{
	// TODO:	REPORT ERROR.		// 11/20/09 AM.
	nlppp->exitpass_ = true;	// 11/20/09 AM.
	return false;					// 11/20/09 AM.
	}

Node<Pn> *node1 = (Node<Pn> *)coll1->start;

if (!node1)	// 11/20/09 AM.
	{
	nlppp->exitpass_ = true;	// Todo: Flag error.	// 11/20/09 AM.
	return false;	// 11/20/09 AM.
	}

// TODO: IF MISSING LAST NODE, CAN BACK UP TO LAST REAL NODE IN COLL.	// 11/20/09 AM.
// BUG: INFINITE LOOP if listadd(1,5) and 5 is optional and missing.		// 11/20/09 AM.
if (!coll2)	// RECOVER.	// 11/20/09 AM.
	{
...
// FIND RIGHTMOST COLLECT ELT WITH A NODE.	// RECOVER	// 11/20/09 AM.
if (!node2)	// RECOVER.	// 11/20/09 AM.
	{
	int x = num2 - 1;									// 11/20/09 AM.
...
if (!node2)	// 11/20/09 AM.
	{
	nlppp->exitpass_ = true;	// Todo: Flag error.	// 11/20/09 AM.
	return false;
	}

// TODO:	LISTADD should robustly handle multi-node elements.	// 11/20/09 AM.
...
//nlppp->red_ = node1;	// Commented.	// 04/25/10 AM.
return true;
}
...
if (!num1 || !num2)
	return false;
//if (num1 > num2)		// Building list to the left.	// FIX	// 09/03/08 AM.
//	return listaddleft(nlppp,num2,num1,keep);				// FIX	// 09/03/08 AM.
...
// Leftmost node is the one whose restart should be retained.	// RECOPT3	// 09/03/08 AM.
// In recursive pass, this eliminates potential infinite loop.	// RECOPT3	// 09/03/08 AM.
pn2->restart_ = pn1->restart_;								// FIX	// RECOPT3	// 09/03/08 AM.
nlppp->rmost_ = nlppp->rightmost_ = node2;							// RECOPT3.	// 09/03/08 AM.
...
Node<Pn> *node1, *node2;
if (!Pat::args_range(num1,num2,nlppp->coll_,nlppp->len_,nlppp,
								/*UP*/ node1,node2))
	return false;
...
{
// NLP++: Handle a badly written group function.	// 09/16/08 AM.
// Recover or just return false?							// 09/16/08 AM.
if (!from || !to)												// 09/16/08 AM.
	return 0;												// 09/16/08 AM.
if (nlppp->nlookahead_)										// 09/16/08 AM.
	...
	if (to >= nlppp->nlookahead_)							// 09/16/08 AM.
		...
		throw ex_EXITPASS;	// 04/14/10 AM.
		return 0;	// 04/14/10 AM.
		}
	}
...
if (!node1 || !node2)
	...
	nlppp->exitpass_ = true;	// RECOVER.	// 04/15/10 AM.
	throw ex_EXITPASS;	// 04/15/10 AM.
	return 0;		// Error to be able to fetch one but not the other.
	}

_TCHAR *name1=0;	// 07/24/07 AM.
nlppp->parse_->internStr(name, /*UP*/name1);	// Intern str.	// 07/24/07 AM.

// Build the group node.
Node<Pn> *node = Pat::group(node1,node2,name1,nlppp);
...
if (!node)								// 04/04/10 AM.
	return 0;							// 04/04/10 AM.
RFASem *sem = new RFASem(node);	// 04/04/10 AM.
return sem;								// 04/04/10 AM.
}
...
// Degenerate case.	// 04/25/10 AM.
RFASem *Arun::group(																// 04/25/10 AM.
	Nlppp *nlppp,
	int from,			// Start of range.
	int to,				// End of range.
...
// Setup restart for recursive passes.					// FIX.		// 09/11/07 AM.
if (nafter)														// FIX.		// 09/11/07 AM.
	{
	Pn *x = nafter->getData();								// FIX.		// 09/11/07 AM.
	Pn *y = node1->getData();								// FIX.		// 09/11/07 AM.
	x->restart_ = y->restart_;								// FIX.		// 09/11/07 AM.
	}
...
if (nlppp->first_ == node1 && nlppp->last_ == node2)				// 03/31/01 AM.
	...
	nlppp->rmost_ = nlppp->rightmost_ = nbefore;		// RECOPT2.	// 08/08/08 AM.
	}
else if (nlppp->first_ == node1)											// 03/31/01 AM.
	{
	nlppp->first_ = nafter;													// 03/31/01 AM.
	nlppp->rmost_ = nlppp->rightmost_ = nafter;		// RECOPT2.	// 08/08/08 AM.
	// Assure no pointer to excised nodes.				// RECOPT2.	// 08/08/08 AM.
	nafter->getData()->restart_ =							// RECOPT2.	// 08/08/08 AM.
							node1->getData()->restart_;	// RECOPT2.	// 08/08/08 AM.
	}
...
Node<Pn> *node1, *node2;
if (!Pat::args_range(num1,num2,nlppp->coll_,nlppp->len_,nlppp,/*UP*/ node1,node2))
	return false;
...
if (!ord || !name || !*name || !nlppp)
	return false;
RFASem *sem = group(nlppp,ord,ord,name);	// 04/04/10 AM.
if (sem)				// 04/04/10 AM.
	{
...
{
Node<Pn> *nd = (Node<Pn> *) node;
// LOOKS UNIMPLEMENTED.	// FIX	// 08/16/08 AM.
return false;
}
...
{
// DEPENDS ON RECURSIVE vs PATTERN, etc.	// PARSESTATE.	// 06/03/13 AM.
//nlppp->start_ = nlppp->node_ = nlppp->after_;	// Set up for next rule. // Comm // 06/03/13 AM.
...
nlppp->rightmost_ = nlppp->rmost_ = 0;					// RECOPT3.	// 08/10/08 AM.


// Todo: Anticipates going to another rule next.
...
switch(val->getType())
	...
	case RSNUM:		// "numeric" string. old stuff.
		...
		_TCHAR *x = val->getName();							// 01/09/07 AM.
		if (typ != LOCALVAR										// 01/09/07 AM.
			 && x && *x)											// 01/16/07 AM.
			nlppp->parse_->internStr(x,						// 01/09/07 AM.
													/*UP*/ x);		// 01/09/07 AM.
		Var::setVal(pair, x);									// 01/09/07 AM.
		}
		break;
	case RSLONG:
...
{
if (!sem)																		// 03/13/02 AM.
	return true;	// Dummy recovery.									// 09/16/08 AM.
...
if (!sem1)															// FIX	// 08/10/08 AM.
	{
	switch (sem2->getType())									// FIX	// 08/10/08 AM.
		{
		case RSLONG:												// FIX	// 08/10/08 AM.
			num = (0 > sem2->getLong());						// FIX	// 08/10/08 AM.
			break;
		case RSFLOAT:												// FIX	// 08/10/08 AM.
			num = (-0.00001 > sem2->getFloat());			// FIX	// 08/10/08 AM.
			break;
		default:														// FIX	// 08/10/08 AM.
			num = 0;													// FIX	// 08/10/08 AM.
			break;
		}
	delete sem2;													// FIX	// 08/10/08 AM.
	return num;														// FIX	// 08/10/08 AM.
	}
if (!sem2)															// FIX	// 08/10/08 AM.
	{
	switch (sem1->getType())									// FIX	// 08/10/08 AM.
		{
		case RSLONG:												// FIX	// 08/10/08 AM.
			num = (sem1->getLong() > 0);						// FIX	// 08/10/08 AM.
			break;
		case RSFLOAT:												// FIX	// 08/10/08 AM.
			num = (sem1->getFloat() > -0.00001);				// FIX	// 08/10/08 AM.
			break;
		default:														// FIX	// 08/10/08 AM.
			num = 1;													// FIX	// 08/10/08 AM.
			break;
		}
	delete sem1;													// FIX	// 08/10/08 AM.
	return num;														// FIX	// 08/10/08 AM.
	}
...
switch (sem->getType())													// 06/11/00 AM.
	...
	case RS_KBVAL:
		...
		break;
	case RSOSTREAM:	// 09/27/11 AM.
		res = (sem->getOstream() ? 1 : 0);	// 09/27/11 AM.
		break;
	case RSPNODE:	// 09/27/11 AM.
		res = (sem->getPnode() ? 1 : 0);	// 09/27/11 AM.
		break;
...
//		else if (*arr2 > *arr1)	// 09/20/06 AM.
		else if (*arr2 < *arr1)	// BIG FIX.	// 05/15/08 AM.
			*ptr++ = *arr2++;
