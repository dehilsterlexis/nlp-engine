...
typedef PTR_ID VAL_ID;	// 02/09/07 AM.
#endif
// Note: A phrase NODE is a specialized CONCEPT.

// Forward reference.
//class NLP;
class AQKBM;	// 06/11/02 AM.
class ALIST;																	// 08/14/02 AM.
class AST;
class APTR;
class ASYM;
class ACON;
class CGATTR;	// 02/18/07 AM.
class CGDICT;	// 02/18/07 AM.
class CGCON;	// 02/19/07 AM.
...
public:
	...
	bool  getLoaded();                                          // 03/04/07 AM.
	AQKBM  *getKBM();															// 06/11/02 AM.
   ...
   static void deleteCG(CG*);                                  // 07/28/03 AM.
	...
	// Save kbms built on qdbm.   // 03/04/07 AM.
	bool saveKBMS();              // 03/04/07 AM.
	...
	bool findVal_id(CONCEPT *con,_TCHAR *attr, /*UP*/ STR_ID &val);	// 02/14/07 AM.
	_TCHAR *strVal(CONCEPT *conc, _TCHAR *s_attr);						// 11/12/00 AM.
	bool CG::isValConcept(VAL *&vals);						// 06/03/02 Dd.
	...
	CONCEPT *CG::findWordConcept(_TCHAR*);								// 06/29/03 AM.
	...
	bool rmNode_cid(PHRASE *&phr, CONCEPT *node);
	...
	bool CG::sort_hier_rec(CONCEPT *tree, bool isroot);         // 02/20/09 AM.

	bool writeTree(															// 04/28/99 AM.
		XCON_S *root,                                            // 03/08/07 AM.
   ...
   void noopCon(CONCEPT *conc);                                // 03/01/07 AM.

	// Class-wide
	static bool goodKbfile(_TCHAR*);										// 08/06/01 AM.

	// Create analyzer sequence.  Allow a header comment.
	//LIBQCONSH_API static bool makeSeq(NLP *nlp, char *comment);

	// Store KB Manager object.
	AQKBM *qkbm_;
	...
	// Attribute module manager.											// 02/16/07 AM.
	CGATTR *cgattr_;															// 02/16/07 AM.

	// Dictionary module manager.											// 02/18/07 AM.
	CGDICT *cgdict_;

	// Concept functions manager.											// 02/19/07 AM.
	CGCON *cgcon_;																// 02/19/07 AM.

	// Moving some KB globals here.										// 08/22/02 AM.
	CON_ID cg_CONCEPT;
	CON_ID con_SYS;															// 02/19/07 AM.
	CON_ID sys_DICT;															// 02/19/07 AM.
	CON_ID dict_ALPHA;														// 02/19/07 AM.
	CON_ID nlp_PUNCT;
	CON_ID nlp_WHT;
...
private:
	...
	bool loaded_;	// If kb has been successfully loaded.			// 03/04/07 AM.
